...
a)  Create a working R package that performs the task it is intended to
    ...
    sim <- simulate_forest(iris, target = "Species", ntree = 150, test_fraction = 0.3)
    head(sim)
    plot_accuracy_growth(sim)                # should open a line plot
    ``` 
    
    
    ``` {r, run multiple random Forests in paralel}
    sim_multi <- simulate_forest_multi(iris, target = "Species", ntree = 150)
    head(sim_multi)
    plot_accuracy_growth_multi(sim_multi)                # should open a line plot
    ``` 
